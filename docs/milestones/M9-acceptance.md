...
resume/rebuild is provided by M9 Task 23, and explicit terminal action retry is
provided by M9 Task 24; action skip, device rejoin, and leader promotion remain
...
The Appium driver/device discovery blocker is resolved for the current environment. Appium-only operation is explicit and does not silently weaken the strict Bridge default. The physical fault/recovery acceptance evidence is recorded in [M9 Task 22](M9-task22-fault-recovery-acceptance.md), the explicit resume/rebuild contract in [M9 Task 23](M9-task23-explicit-resume-acceptance.md), and the explicit action retry contract in [M9 Task 24](M9-task24-action-retry-acceptance.md). M10 remains independently accepted; action skip, device rejoin, and leader promotion are separate follow-up scope.
