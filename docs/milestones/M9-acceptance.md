...
quarantined-follower rejoin is provided by M9 Task 28. Leader promotion remains
...
The Appium driver/device discovery blocker is resolved for the current environment. Appium-only operation is explicit and does not silently weaken the strict Bridge default. The physical fault/recovery acceptance evidence is recorded in [M9 Task 22](M9-task22-fault-recovery-acceptance.md), the explicit resume/rebuild contract in [M9 Task 23](M9-task23-explicit-resume-acceptance.md), the explicit action retry contract in [M9 Task 24](M9-task24-action-retry-acceptance.md), the console pause/resume controls in [M9 Task 25](M9-task25-session-controls-acceptance.md), the console action retry surface in [M9 Task 26](M9-task26-action-retry-console-acceptance.md), explicit Action Skip in [M9 Task 27](M9-task27-action-skip-acceptance.md), and paused quarantined-follower rejoin in [M9 Task 28](M9-task28-device-rejoin-acceptance.md). M10 remains independently accepted; Leader promotion is separate follow-up scope.
