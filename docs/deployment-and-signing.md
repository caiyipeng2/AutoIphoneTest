...
The Apps page can run `POST /api/artifacts/build/validate` before a Unity build. This preflight checks the selected provider and import-root paths without starting Unity; a failed validation is shown in the dialog and no artifact is published.
